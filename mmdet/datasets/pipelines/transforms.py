...
class Resize(object):
    ...
    def _resize_img(self, results):
        ...
        if 'template' in results:
            template = mmcv.imrescale(
                    results['template'], results['scale'], return_scale=True)[0]
            results['template'] = template

        if 'normal' in results:
            normal = mmcv.imrescale(
                    results['normal'], results['scale'], return_scale=True)[0]
...
class RandomFlip(object):
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            if 'template' in results:
                results['template'] = mmcv.imflip(
                        results['template'], direction=results['flip_direction'])

            if 'normal' in results:
                results['normal'] = mmcv.imflip(
                        results['normal'], direction=results['flip_direction'])
...
class Pad(object):
    ...
    def _pad_img(self, results):
        ...
        if 'template' in results:
            if self.size is not None:
                padded_template = mmcv.impad(results['template'], self.size, self.pad_val)
            else:
                padded_template = mmcv.impad_to_multiple(
                        results['template'], self.size_divisor, pad_val=self.pad_val)
            results['template'] = padded_template

        if 'normal' in results:
            if self.size is not None:
                padded_normal = mmcv.impad(results['normal'], self.size, self.pad_val)
            else:
                padded_normal = mmcv.impad_to_multiple(
                        results['normal'], self.size_divisor, pad_val=self.pad_val)
            results['normal'] = padded_normal
...
class Normalize(object):
    ...
    def __call__(self, results):
        ...
        if 'normal' in results:
            results['normal'] = mmcv.imnormalize(results['normal'], self.mean, self.std, self.to_rgb)
