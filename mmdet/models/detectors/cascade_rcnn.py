...
class CascadeRCNN(BaseDetector, RPNTestMixin):
    ...
    def extract_feat(self, img, template=None):
        x = self.backbone(img)
        if template is not None:
            xt = self.backbone(template)
            x = [i1 - i2 for i1, i2 in zip(x, xt)]
        if self.with_neck:
            x = self.neck(x)
        return x
...
@DETECTORS.register_module
class CascadeRCNN_PTG(CascadeRCNN):
    ...
    def __init__(self,
        ...
        super(CascadeRCNN_PTG, self).__init__(
    ...
    def forward_train(self,
        ...
        for i in range(self.num_stages):
            ...
            if self.context_reference > 0:
                ...
                context_feats = bbox_roi_extractor(
                        x[:bbox_roi_extractor.num_inputs], rois_context)
                bbox_feats = torch.cat((bbox_feats, context_feats), dim=1)
    ...
    def simple_test(self, img, img_metas, proposals=None, rescale=False):
        ...
        for i in range(self.num_stages):
            ...
            if self.context_reference > 0:
                ...
                context_feats = bbox_roi_extractor(
                        x[:len(bbox_roi_extractor.featmap_strides)], rois_context)
                bbox_feats = torch.cat((bbox_feats, context_feats), dim=1)
